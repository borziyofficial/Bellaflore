...
export function AdminNavigationShell({
  ...
  const handleLogout = () => {
    logoutAdminEntrySession();
    router.replace(ADMIN_ENTRY_LOGIN_PATH);
  };
        ...
        <div className={styles.headerInner}>
          <div className={styles.headerTop}>
            <div className={styles.brandBlock}>
              ...
              <h1 className={styles.title}>{activeItem?.label ?? "Admin"}</h1>
              <p className={styles.sectionTitle}>
                {activeItem?.description ?? "Internal admin app"}
              </p>
            </div>
            ...
            <div className={styles.userBlock}>
              <p className={styles.userMeta}>
                ...
                <span className={styles.userName}>
                  {adminUser?.adminUserName ?? session?.userName ?? "—"}
                </span>
                {" / "}
          ...
          <nav className={styles.nav} aria-label="Admin entry navigation">
